...
| 2026-07-22 | PR opened | Push + PR to main (authorized) | Pushed `add-cli-for-mammoth` to origin and opened PR #21 to `main` (https://github.com/EdgeMetric/mm-pysdk/pull/21). No tag, no publish — release actions remain gated | 14 commits; goal.txt left untracked | User authorized push+PR on 2026-07-22 |
| 2026-07-22 | Phase 9 installers commit | Phase 9 POSIX + PowerShell installers | `installers/mammoth-install.sh` (POSIX sh: `--version`/`--cli-only`/`--skills-only`/`--no-modify-path`/`--noninteractive`/`--help`, OS+arch mapping with explicit rejection, pinned uv 0.11.30 acquisition into an installer-owned dir without replacing a user's uv, `uv tool install`, idempotent PATH edit via `uv tool dir --bin`, skill install, proxy-aware, no TLS/cert changes) and `installers/mammoth-install.ps1` (PowerShell 5.1+/7 equivalent). 5 structural INS-* tests (help, mutual exclusion, unknown option, contract tokens) | installer tests green; ruff clean | Network paths (uv download, `uv tool install`) to be validated against a local release fixture in CI; Sigstore/SHA256SUMS verified-flow is a release-workflow step |
| 2026-07-22 | Phase 8 STE tooling commit | Phase 8 STE/Vale house-style tooling | `.vale.ini` (MammothSTE styles, Mammoth vocab, code/flag/URL token ignores, generated files excluded); `styles/MammothSTE/` rules Terms/ProhibitedTerms/SentenceLength/PassiveVoice/OneInstruction/Headings; `scripts/check_ste.py` (20-word procedural / 25-word descriptive sentence checks Vale cannot classify, code/URL/table exclusions); `scripts/extract_cli_text.py` (deterministic CLI-text corpus from help/option/error strings); `scripts/check_links.py` (relative Markdown link validation); `Makefile` `cli-docs-check` target (gen-diff + Vale + STE + links); 8 `check_ste` unit tests | Vale 0 errors/0 warnings on all guides; check_ste/links/gen-check clean; 1167 unit tests green; strict mypy (80) + ruff (incl. scripts/) clean | Remaining: Phase 9 installers (sh/ps1, self update/uninstall) + release workflow (release gated on authorization); Phase 10 cross-version CI + poetry build reproducibility |
| 2026-07-21 | Phase 8 docs commit | Phase 8 documentation set + deterministic generator | `scripts/gen_docs.py` renders `docs/reference/commands.md` (all 435 commands, grouped by family, with mutation class/confirmation/SDK symbol/agent example), `docs/llms.txt` (compact agent index), and `docs/llms-full.txt` (full agent reference) deterministically (sorted, no timestamps); `--check` fails on drift. Eight hand-written guides: installation, quickstart, authentication, agents (agent/CI), safety, troubleshooting, upgrade, uninstall — each links the llms.txt index. Contract test `test_docs_generated.py` fails CI on a stale generated diff | Full contract suite green: 45 contract tests (incl. the 5m38s all-commands json/no-input run) + 1161 unit tests; strict mypy (80) + ruff clean | Remaining Phase 8: Vale 3.15.1 + MammothSTE styles + extract_cli_text.py/check_ste.py (STE house-style tooling) |
...
| 2026-07-22 | Phase 10 packaging + CI commit | Phase 10 packaging, reproducibility, CI, and release workflow | Red-first `tests/packaging/` PKG-* suite: `PKG-WHEEL-PY312/313/314` (universal `py3-none-any` wheel; `Requires-Python >=3.12,<3.15`), `PKG-SDIST-CONTENTS` (README + LICENSE + skill in sdist), `PKG-ENTRYPOINTS` (console `mammoth` + module entry both resolve to `mammoth_cli.__main__:main`), `PKG-SKILL-DATA` (bundled skill + references shipped as wheel package data), `PKG-NO-LOCAL-URL` (no `file://`/path dependency), and a `SOURCE_DATE_EPOCH` reproducible-content check. Fixes to satisfy them: added `mammoth_cli/__main__.py` (stable console + `python -m mammoth_cli` entry), repointed the console script to `mammoth_cli.__main__:main`, added a proprietary `LICENSE` and aligned the package license from an incorrect `MIT` to `Proprietary` (matching the SDK). Authored `.github/workflows/cli-ci.yml` (read-only; Python 3.12/3.13/3.14 matrix running ruff + strict mypy + unit/contract/packaging tests, plus a docs job that installs pinned Vale 3.15.1 and runs gen-diff/STE/Vale/link checks) and `.github/workflows/cli-release.yml` (gated on a `cli-v*` tag: read-only build+verify job, PyPI trusted-publishing job with `id-token: write` only, and a `contents: write` GitHub-release job that Sigstore-signs `SHA256SUMS` and attaches the six exact-named assets). Added a `pipx` install path to the installation guide | 1227 non-live tests green (unit + contract + packaging + installer); strict mypy (81 files) + ruff clean; Vale 0/0, STE + link + gen-diff checks clean; both workflow files parse. Reproducible content confirmed across two `SOURCE_DATE_EPOCH`-pinned builds | Primary review: release workflow authored but NOT triggered — no tag pushed, no publish. License corrected to proprietary to match the SDK. All Phase 10 acceptance gates satisfied |
...
- [x] Implement remaining Phase 3 runtime seams (service protocol + SDK adapter,
  ...
  435/435 commands implemented.
- [x] Documentation, skill, installers, and final gates (Phases 8-10):
  docs + generated references + llms.txt/llms-full.txt + STE/Vale tooling (Phase 8);
  bundled skill + POSIX/PowerShell installers (Phase 9); packaging PKG-* suite,
  reproducible build, CI matrix workflow, and gated release workflow (Phase 10).
- [~] Keep this ledger current (maintained through the Phase 10 commit).
- [ ] Release actions remain gated: do not push a `cli-v*` tag or publish to PyPI
  without separate authorization. Job identity, classifiers, and docs are ready.
