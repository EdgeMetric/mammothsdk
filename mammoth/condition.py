...
Conditions are resolved to API format via build(column_map):
...
class Condition:
    ...
    Args:
        ...
        value: Comparison value. Required for most operators, omit for IS_EMPTY/IS_NOT_EMPTY.
    ...
    def __init__(
        ...
        self.column = column
        self.operator = operator.value if hasattr(operator, "value") else str(operator)
        self.value = value
    ...
    def build(self, column_map: Optional[Dict[str, str]] = None) -> dict:
        """Build API-format condition dict.

        Args:
            column_map: Mapping of display names to internal names.
                        e.g. {"Sales": "column_1"}. If None, uses column name as-is.
        ...
        if self.operator in ("IS_EMPTY", "IS_NOT_EMPTY", "IS_MAXVAL", "IS_NOT_MAXVAL",
        ...
        # String operators
        if self.operator in ("STARTS_WITH", "ENDS_WITH", "NOT_STARTS_WITH", "NOT_ENDS_WITH"):
...
class CompoundCondition:
    ...
    def build(self, column_map: Optional[Dict[str, str]] = None) -> dict:
        ...
        for cond in self.conditions:
            built.append(cond.build(column_map))
