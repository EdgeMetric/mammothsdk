...
| Decide on every blank the deliverable uses | one report line for each `blank_values` entry (from the upload, a data read or `deliverable_check`), in every view: the column, the count, and filled, filtered, or kept and why. Kept is a decision only when you write it down |
