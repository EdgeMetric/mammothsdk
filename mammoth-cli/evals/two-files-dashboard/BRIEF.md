...
## Setup rules
- Install the release under test from PyPI into a fresh virtual environment: `python3 -m venv WORK_DIR/venv && WORK_DIR/venv/bin/pip install --no-cache-dir mammoth-cli==VERSION_UNDER_TEST`. Confirm `mammoth --version` prints VERSION_UNDER_TEST before anything else. Use that environment's `mammoth` binary by absolute path for every command. Then load its guide with `mammoth skill show` and follow it.
