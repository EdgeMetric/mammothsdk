...
- `ilg-prague-20260921/` — the ILG shape rebuilt on a second environment (prague ws 4) with fresh mock data and the production procedure: `--dry-run` before every write, `expected_task_count` on every pipeline write, a stale count refused, prague's `need_action` upload resolved through `dataset file-settings`, countDistinct 98 / 55 (Q1) / 97 (after an upstream removal) against the key; 126 commands, 68/68 real checks. `run.jsonl`, `results.jsonl`, `fixtures/`, `key.json`, `run.py`, `SUMMARY.md`.
- `dryrun-precondition-20260920/` — `--dry-run` across command shapes (nothing sent, resolved call reported), `expected_task_count` refusing a stale pipeline edit with `pipeline_changed`, `extract-date` and `window` read back against local recomputation, bundled-skill hygiene sweep.
