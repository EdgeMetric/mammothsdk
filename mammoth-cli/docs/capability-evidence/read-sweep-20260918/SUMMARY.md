...
- **`dashboard.og-card`** returns a non-JSON (likely binary image) body with status 200, but the CLI unconditionally tries to JSON-decode responses — this will break for every non-JSON-returning route, not just this one.
- **Several list-vs-get id mismatches**: `connector.get` rejects a connector_key that `connector.list` just returned; `workspace.user.get` 405s for a user id that `workspace.user.list` returns cleanly; `browse.folder` rejects folder id `0`, which is exactly the id `folder.root` reports for the root folder.
