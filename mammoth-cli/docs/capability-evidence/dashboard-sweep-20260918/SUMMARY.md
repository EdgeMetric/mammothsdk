...
4. **Two documented `runnable_example`s do not run as printed.**
   ...
   - `dashboard pdf export 52 --input '{"body": {"params": {"data": {}}}}'` is likewise the documented example; it fails with HTTP 400 telling you `data` must be a hydrated `DashboardData` map, which `{}` obviously is not, and which the schema gives no way to construct.

5. **`dashboard data draft` / `dashboard data published` schemas omit the request envelope the backend actually requires.** The schema's `input_schema` and `runnable_example` both say the body is just `{"sql": "..."}` at the top level. The backend rejects this with HTTP 400 `4GENR007 VALIDATION_ERROR`, `"key": "params", "message": "Field required"` — i.e. it wants `{"params": {"sql": ...}}` like every other dashboard sub-command, but the schema for these two commands never says so.
