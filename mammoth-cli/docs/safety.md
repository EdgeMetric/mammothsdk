...
A dry run is a full process with the same reads as the real run. Budget
about 2 s per request on a hosted environment, for TLS setup from a cold
process plus server time. So `view task list` → `--dry-run` → write takes
about 30–40 s per pipeline step. That is the cost of the procedure, not a hang.
