...
def test_condition_generate_invalid_dataset_id(fake_service: FakeMammothService) -> None:
    with pytest.raises(CliError) as excinfo:
        ai_cmd.ai_condition_generate(_inv("ai.condition.generate", project=180, extra_args=["abc"]))
    assert excinfo.value.code == "invalid_argument"


def test_condition_generate_requires_intent(fake_service: FakeMammothService) -> None:
    with pytest.raises(CliError) as excinfo:
        ai_cmd.ai_condition_generate(_inv("ai.condition.generate", project=180, extra_args=["12"]))
    assert excinfo.value.code == "missing_field"
    assert fake_service.call_log == []


def test_condition_generate_minimal_call(fake_service: FakeMammothService, tmp_path: Path) -> None:
...
def test_expression_generate_minimal_call(fake_service: FakeMammothService, tmp_path: Path) -> None:
