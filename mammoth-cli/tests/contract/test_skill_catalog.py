...
def test_packaged_skill_catalog_is_manifest_complete_and_current() -> None:
    completed = subprocess.run([sys.executable, str(SCRIPT), "--check"], cwd=ROOT, text=True, capture_output=True)
    assert completed.returncode == 0, completed.stdout + completed.stderr
    catalog = "\n".join(path.read_text(encoding="utf-8") for path in (SKILL / "references" / "commands").glob("*.md"))
